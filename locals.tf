locals {
  ...
  # SSH connection
  ssh_public_key  = var.generate_ssh_keys ? chomp(tls_private_key.ssh_key[0].public_key_openssh) : chomp(file(var.ssh_public_key))
  ssh_private_key = var.generate_ssh_keys ? chomp(tls_private_key.ssh_key[0].private_key_pem) : chomp(file(var.ssh_private_key))
